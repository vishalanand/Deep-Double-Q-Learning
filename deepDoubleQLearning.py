...
class DoubleDeepQLearning:
  ...
  def createNetwork(self, s_net):
    ...
    W_fc2 = self.weight_variable([512, self.argsPassed.action_count])
    b_fc2 = self.bias_variable([self.argsPassed.action_count])

    # input layer
    #s_net = tf.placeholder("float", [None, 80, 80, 4])

    # hidden layers (with weights and biases)
    h_conv1 = tf.nn.relu(self.conv2d(s_net, W_conv1, 4) + b_conv1)
  ...
  def playGame(self):
    ...
    #Player 1
    readout_net1, h_fc1_net1 = self.createNetwork(s)
    readout_net2, h_fc1_net2 = self.createNetwork(s)
    
    #Player 2
    readout_netb1, h_fc1_netb1 = self.createNetwork(s)
    readout_netb2, h_fc1_netb2 = self.createNetwork(s)

    self.trainNetwork(s, readout_net1,readout_net2, readout_netb1,readout_netb2, sess)
